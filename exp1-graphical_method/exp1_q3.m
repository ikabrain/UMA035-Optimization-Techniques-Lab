...
if isempty(points)
    ...
    [max_z, max_idx] = max(z);
    [min_z, min_idx] = min(z);
    max_x1 = points(max_idx, 1);
    max_x2 = points(max_idx, 2);
    min_x1 = points(min_idx, 1);
    min_x2 = points(min_idx, 2);
    fprintf("Maximum value is %.2f at (%.2f, %.2f)\n", max_z, max_x1, max_x2);
    fprintf("Minimum value is %.2f at (%.2f, %.2f)\n", min_z, min_x1, min_x2);
    
    plot(max_x1, max_x2, "kp", "MarkerSize", 12, "MarkerFaceColor", "y", "DisplayName", "Maximum Value");
    text(max_x1, max_x2, sprintf("    (%.2f, %.2f)", max_x1, max_x2));

    plot(min_x1, min_x2, "kp", "MarkerSize", 12, "MarkerFaceColor", "m", "DisplayName", "Minimum Value");
    text(min_x1, min_x2, sprintf("    (%.2f, %.2f)", min_x1, min_x2));
end